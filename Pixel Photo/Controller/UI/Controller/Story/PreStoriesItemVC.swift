...
class PreStoriesItemVC: BaseVC {
    ...
    var pageIndex : Int = 0
    var item: [FetchStoryModel.Story] = []
    var items: [FetchStoryModel.Datum] = []
    ...
    override func viewDidLoad() {
        ...
        if items[pageIndex].userID != AppInstance.instance.userId ?? 0 {
    ...
    private func getDuration(at index: Int) -> TimeInterval {
        ...
        if item[index].type == "1" {
            ...
            guard let url = NSURL(string: item[index].mediaFile!) as URL? else { return retVal }
    ...
    func playVideoOrLoadImage(index: Int) {
        ...
        if item[index].type == "1" {
            ...
            self.imagePreview!.sd_setImage(with: URL(string: item[index].mediaFile!)) { (image, error, cacheType, url) in
            ...
            resetPlayer()
            guard let url = NSURL(string:  item[index].mediaFile!) as URL? else {return}
