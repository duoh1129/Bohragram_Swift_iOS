...
import Rswift
class PPMultiImageItemTableViewCell: UITableViewCell,EditPostDelegate {
...
//            self.typeImgView.image = UIImage(named: "ic_type_image")
    ...
    func bind(item : ShowPostModel){
        ...
        if loaded == false {
            ...
            self.images = item.MediaUrlsArray ?? [""]
            print(images)
            self.imageCollectionView.reloadData()
...
//            cell.bind(item:(self.showPostModel?.MediaUrlsArray![indexPath.row])!, type: "image")
            cell.bind(item:(self.images[indexPath.row]), type: "image")
        } else if self.homeVC != nil{
